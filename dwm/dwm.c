...
struct Pertag {
	unsigned int curtag; /* 1..LENGTH(tags); 0 = all tags visible */
...
static void pertag_apply(Monitor *m);
...
createmon(void)
{
	...
	if (!(m->pertag = calloc(1, sizeof(Pertag))))
		die("fatal: could not malloc() %u bytes\n", sizeof(Pertag));
...
static void
pertag_apply(Monitor *m)
{
	unsigned int t = m->pertag->curtag;

	m->nmaster = m->pertag->nmasters[t];
	m->mfact = m->pertag->mfacts[t];
	m->sellt = m->pertag->sellts[t];
	m->lt[m->sellt] = m->pertag->ltidxs[t][m->sellt];
	m->lt[m->sellt ^ 1] = m->pertag->ltidxs[t][m->sellt ^ 1];
	strncpy(m->ltsymbol, m->lt[m->sellt]->symbol, sizeof m->ltsymbol);
	if (m->showbar != m->pertag->showbars[t])
		togglebar(NULL);
}
