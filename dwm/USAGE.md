...
| 状态栏字体 | JetBrainsMono Nerd Font | 需 Nerd Font 才能正确显示图标 |
...
| **Super + p** | ⚠️ 与下方 Gapless 冲突，实际生效为 **Zoom**（当前窗口升至 master） |
...
> **注意：** `Super + p` 在 `config.h` 中绑定了两次（Gapless 布局与 Zoom），通常后注册的 **Zoom** 会生效。若需 Gapless 布局，请修改配置消除冲突。
...
### Gapless `:::` — 无缝网格（**Super + p**，与 Zoom 冲突）
...
| 状态栏脚本逻辑 | 编辑 `dwmblocks/bin/*` → 复制到 PATH → `kill -(34+signal) $(pidof dwmblocks)` |
...
完整补丁列表见仓库根目录 [README.md](../README.md)。
